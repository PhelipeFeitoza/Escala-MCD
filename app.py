...
<style>
    ...
    table { font-weight: bold !important; color: black !important; width: 100%; border-collapse: collapse; }
    th, td { border: 1px solid #999 !important; padding: 8px !important; text-align: center !important; }
</style>
...
if os.path.exists(NOME_ARQUIVO):
    ...
    if menu == "📈 Painel de Produtividade":
        ...
        with c_res:
            ...
            v = len(df_dash[df_dash['ST_LIMPO'] == 'VAGA']); f = len(df_dash[df_dash['ST_LIMPO'].isin(['FT', 'FALTA'])]); l = len(df_dash[df_dash['ST_LIMPO'].isin(['LM', 'LICENÇA MÉDICA'])]); fr = len(df_dash[df_dash['ST_LIMPO'].isin(['FR', 'FÉRIAS'])])
            ...
            st.markdown(f'<div class="metric-row folga">FOLGA / BH <span>{folgas}</span></div>', unsafe_allow_html=True); st.markdown(f'<div class="metric-row tr">TREINAMENTO <span>{tr}</span></div>', unsafe_allow_html=True); st.markdown(f'<div class="metric-row pv">PREVENTIVA <span>{pv}</span></div>', unsafe_allow_html=True); st.markdown(f'<div class="metric-row trabalhando">EQUIPE TRABALHANDO <span>{ativos}</span></div>', unsafe_allow_html=True)
        ...
        with c_det:
            ...
            for label, busca in [('SÃO PAULO', 'SÃO PAULO'), ('INTERIOR', 'INTERIOR'), ('SUL', 'SUL'), ('NORDESTE', 'NORDESTE')]:
                ...
                ca, cb, cc = st.columns([1.2, 1, 1]); ca.markdown(f'<div class="metric-row folga">HEADCOUNT {label} <span>{h_r}</span></div>', unsafe_allow_html=True); cb.markdown(f'<div class="metric-row headcount">CALL RATE {label} <span>{c_r}</span></div>', unsafe_allow_html=True)
            ...
            def get_color(at, mt):
                r = at/mt if mt > 0 else 0
                if r >= 1.0: return "#c8e6c9", "✅"
                if r >= 0.5: return "#fff9c4", "⚠️"
                return "#ffcdd2", "🚨"
            c_sp = st.columns(3)
            for i, (sub, mt) in enumerate(sub_sp.items()):
                val = int(df_dash[(df_dash['Grupo_Regiao'] == 'SÃO PAULO') & (df_dash['Regiao'].str.contains(sub))]['Is_Ativo'].sum())
                bg, icon = get_color(val, mt)
                c_sp[i%3].markdown(f'<div class="sub-sp-card" style="background-color: {bg};"><span>{icon} {sub}</span><span>{val} / {mt}</span></div>', unsafe_allow_html=True)

    # --- DEMAIS VISÕES ---
    elif menu == "📅 Escala Semanal":
        dt_ini = st.sidebar.date_input("Início da semana:", data_sel)
        df_s = df_base[(df_base['Data'].dt.date >= dt_ini) & (df_base['Data'].dt.date <= dt_ini + timedelta(days=6))]
        ...
        m_sel = st.sidebar.slider("Mês:", 1, 12, int(data_sel.month))
        df_m = df_base[df_base['Data'].dt.month == m_sel]
        ...
        for r in range(len(df_grid)):
            for c in range(7):
                ...
                if dia != 0:
                    status = df_tec_mes[df_tec_mes['Data'].dt.day == dia]['Status'].values
                    txt = status[0] if len(status) > 0 else "---"
                    df_grid.iloc[r, c] = f"{dia} - {txt}"
                else: df_grid.iloc[r, c] = ""
        st.table(df_grid.style.map(style_status))

    elif menu == "📋 Espelho de Ponto":
        t_p = st.selectbox("Técnico:", sorted(df_base['Tecnico'].unique()))
        m_f = st.sidebar.selectbox("Fechamento (Dia 27):", range(1, 13), index=int(data_sel.month)-1)
        fim = datetime(2026, m_f, 27); ini = (fim - timedelta(days=31)).replace(day=28)
        df_p = df_base[(df_base['Tecnico'] == t_p) & (df_base['Data'].dt.date >= ini.date()) & (df_base['Data'].dt.date <= fim.date())]
